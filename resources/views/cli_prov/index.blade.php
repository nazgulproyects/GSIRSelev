        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="row d-flex mb-2">
                ...
                <div class="col-6 d-flex justify-content-end">
                    <x-button type="button" onclick="$('#modalNuevoCliProv').modal('show');">Nuevo Cliente/Prov</x-button>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                    <table id="table" class="display">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><x-nav-link href="/cli_prov/{{$c_p->id}}">{{ str_pad($c_p->id, 4, '0', STR_PAD_LEFT) }}</x-nav-link></td>
                                <td>{{$c_p->nombre}}</td>
                                <td>@if($c_p->fecha_antiguedad != null) {{ \Carbon\Carbon::parse($c_p->fecha_antiguedad)->format('d/m/Y') }} @endif</td>
                                <td>{{$c_p->tramo_actual_comision}}</td>
                                <td><x-danger-button type="button" onclick="eliminarFila('{{$c_p->id}}');"><i class="fa-solid fa-xmark fa-fade"></i></x-danger-button></td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
