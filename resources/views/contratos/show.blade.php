...
<x-app-layout>
    ...
    <x-nav-link href="{{ route('contratos.index') }}"
        :active="request()->routeIs('contratos.index')">Contratos</x-nav-link>
    <x-nav-link href="#" active="true">{{$contrato->codigo}}</x-nav-link>
    @endsection
    <div class="mt-2">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <form action='/contratos/update/{{$contrato->id}}' method="POST" enctype="multipart/form-data"
                ...
                <div class="row d-flex mb-2">
                    ...
                    <div class="col-6 d-flex justify-content-end">
                        <a href="/contratos/pdf/{{$contrato->id}}" target="_blank" class="boton-pdf">
                            <i class="fa-solid fa-file-pdf"></i> Contrato PDF
                        </a>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="form-group row mb-1">
                            ...
                            <x-label class="col-sm-6 col-form-label">
                                ...
                                @if($contrato->punto_recogida->asignado_a_ruta)
                                    <b style="color: green;"><x-nav-link
                                            href="/rutas/{{$contrato->punto_recogida->asignado_a_ruta}}">Ruta
                                            {{$contrato->punto_recogida->asignado_a_ruta}}</x-nav-link></b>
                                @else
                        ...
                        <div class="form-group row mt-2">
                            ...
                            <div class="col-sm-10">
                                <x-input type="date" name="fecha_recogida_inicial" class="form-control"
                                    value="{{$contrato->fecha_recogida_inicial }}" />
                            </div>
                        </div>
                        <div class="form-group row">
                            <x-label class="col-sm-2 col-form-label">Próxima fecha recogida:</x-label>
                            <div class="col-sm-10">
                                <x-input type="date" name="prox_dia_recogida" class="form-control"
                                    value="{{$contrato->prox_dia_recogida }}" />
                            </div>
                        </div>
                        <div class="form-group row">
                            <x-label class="col-sm-2 col-form-label">Frecuencia recogida:</x-label>
                            <div class="col-sm-10">
                                <x-input type="number" step="1" name="frecuencia" class="form-control"
                                    value="{{$contrato->frecuencia}}" />
                            </div>
                        </div>
                        <div class="form-group row">
                            <x-label class="col-sm-2 col-form-label">Coste Unitario:</x-label>
                            <div class="col-sm-10">
                                <x-input type="number" step="0.01" name="coste_unitario" class="form-control"
                                    value="{{$contrato->coste_unitario}}"></x-input>
                            </div>
                        </div>
                        <div class="form-group row">
                ...
                <div class="card mt-2">
                    <div class="card-body">
                        <div class="row d-flex mb-2">
                            ...
                            <div class="col-6 d-flex justify-content-end">
                                <x-button type="button" onclick="$('#modalNuevoProdContrato').modal('show');">Añadir
                                    producto</x-button>
                            </div>
                        </div>
                        <table id="table" class="display">
                            ...
                            <tbody>
                                @foreach($productos_contrato as $prod_cont)
                                    <tr>
                                        <td>{{$prod_cont->producto->descripcion}}</td>
                                        <td><x-danger-button type="button" onclick="eliminarFila('{{$prod_cont->id}}');"><i
                                                    class="fa-solid fa-xmark fa-fade"></i></x-danger-button></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<div class="modal" tabindex="-1" role="dialog" id="modalNuevoProdContrato">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action='/contratos/asignar_prod_contrato/{{$contrato->id}}' method="POST"
